...
subroutine find_flowline(output_directory, output_suffix)
    ...
    character(len=255), INTENT(IN) :: output_directory
    character(len=10), INTENT(IN) :: output_suffix

    character(len=255) :: contours_file
    character(len=255) :: contours_rejected_file
    character(len=255) :: oversample_points_file

    character(len=255), parameter :: contours_filename = "contours." 
    character(len=255), parameter :: contours_rejected_filename = "contours-rejected." 
    character(len=255), parameter :: oversample_points_filename = "oversample_points."
    ...
    contours_file = trim(output_directory) // trim(contours_filename) // trim(output_suffix)
    contours_rejected_file = trim(output_directory) // trim(contours_rejected_filename) // trim(output_suffix)
    oversample_points_file = trim(output_directory) // trim(oversample_points_filename) // trim(output_suffix)
    ...
    !			ss = shear_stress(dble(test_x_counter),dble(test_y_counter))
    ...
    initialize_parameters: do initializing_counter = 1, number_boundary_points, 1
        ...
        ss = shear_stress(x_distance(initializing_counter),y_distance(initializing_counter)) 
    ...
    open(unit=500, file=contours_file, access="sequential", form="formatted", status="replace")
    open(unit=501, file=contours_rejected_file, access="sequential", form="formatted", status="replace")
    open(unit=502, file=oversample_points_file, access="sequential", form="formatted", status="replace")
    ...
    contains
        recursive subroutine calculate_polygon5(current_boundary_points, E_array, x_contour, y_contour, &
            ...
            integer :: next_step, index_tracker2, next_index, oversample_count, oversample_array_size, boundary_counter2
            integer :: current_index, boundary_counter, counter2
            ...
            double precision :: dir_cur, y_next, x_next, lon, lat, angle_check, distance_tracker
            double precision :: shear_stress_check, elevation_check
            ...
            integer, allocatable, dimension(:) :: crossover_index1, crossover_index2, index_array_expanded, status_array 
            integer, allocatable, dimension(:) :: crossover_index1_temp, crossover_index2_temp, status_array2
            ...
            logical :: is_crossover, extra_skip_point, delete_first, oversample_done, skip_interpolation
            logical :: end_step_elevation, connect
            ...
            write(6,*) "current step", current_step
            write(6,*) "minval(E_array)", minval(E_array)
            write(6,*) "elevation_interval", elevation_interval
            ...
            boundary_search: do boundary_counter = 1, current_boundary_points, 1
                ...
                elseif(E_array_last(boundary_counter) > current_elevation) THEN ! do not need to calculate the flowline if it is above the current contour
                    ...
                    elseif(.not. point_in_polygon(x_contour_last,y_contour_last,x_next,y_next,current_boundary_points)) then ! outside of the limits of the last polygon, do not include
                        ...
                        H_f = shear_stress(x_next, y_next) / (rho_ice * g)
            ...
            ! after finding all the stepped up flowlines, the next step is to see if those points cross over one another. Using the 
            ! general rules of a motorcycle graph algorithm (e.g. http://arxiv.org/abs/1303.5958), crossover lines are eliminated.
            ! The rule is that if a line crosses over another, the line that has the shorter distance to the crossover point is 
            ! eliminated

            ! worst case scenario is that there is approximately n*log(n) crossovers. This is unlikely to happen except near the 
            ! end of the ice sheet calculation
            ...
            do poly_counter = 1, number_polygons, 1
                ...
                skip_point_temp(1:number_points)) .and. .not. rejected) THEN
                    ...
                    do write_counter = 1, number_points
                        ...
                        ss = shear_stress(x_contour_temp(write_counter),y_contour_temp(write_counter))
                ...
                ! resample the array and call the subroutine again
                    ...
                    oversample_loop: do counter = 1, number_points, 1
                        ...
                        if(nint(E_temp3(oversample_count)) <= nint(current_elevation+elevation_interval)) THEN
                            ...
                            ss = shear_stress(x_contour_temp3(oversample_count),y_contour_temp3(oversample_count))
                        ...
                        check_interval: do
                            ...
                            if(reduction_factor < 0.0000001) THEN ! something has likely messed up
                                ...
                                write(6,*) x_contour_temp(counter), y_contour_temp(counter), E_temp_new(counter), &
                                    shear_stress(x_contour_temp(counter), y_contour_temp(counter))
                                write(6,*) x_contour_temp3(oversample_count), y_contour_temp3(oversample_count), &
                                    E_temp3(oversample_count), shear_stress(x_contour_temp3(counter), y_contour_temp3(counter))
                            ...
                            oversample_SS = shear_stress(oversample_x,oversample_y)
        ...
        ! deallocate everything
            deallocate(x_contour_temp,y_contour_temp,E_temp_new,interior_direction_temp_new, &
                skip_point_temp, status_array,stat=istat)
            if(istat/=0) THEN
                write(6,*) "error deallocating temp arrays"
                stop
            endif

            deallocate (x_poly, y_poly, dir_poly, index_array_expanded, q_poly, E_poly, crossover_point, &
                skip_point_poly,stat=istat)
            if(istat/=0) THEN
        ...
        subroutine step_flow_line(x_start_initial, y_start_initial, current_interior_direction, q, E, dx_l_current, &
                        current_elevation, x_next, y_next, q_next, E_next) 
        ...
        subroutine RK4(x_l, y_l, q, E, dB_dy, dHf_dy, dx_l_current, y_out, q_out, E_out,error,&
            ...
            H_f = shear_stress(x_current,y_current) / (rho_ice*g)
            ...
            Hf_temp1 = shear_stress(x_current,y_current) / (rho_ice*g)
            ...
            Hf_temp2 = shear_stress(x_current,y_current) / (rho_ice*g)
            ...
            Hf_temp3 = shear_stress(x_current,y_current) / (rho_ice*g)
            ...
            H_f = shear_stress(x_current1,y_current1) / (rho_ice*g)
