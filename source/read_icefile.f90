...
subroutine read_icefile(filename)  
    ...
    do counter=1, number_boundary_points_temp-1, 1
        ...
        if (x < elev_x_coords(1) .or. x > elev_x_coords(Ubound(elev_x_coords,1))) then
            write(*,*) x, "exceeds grid val x"
        endif
        
        if (y < elev_y_coords(1) .or. y > elev_y_coords(Ubound(elev_y_coords,1))) then
            write(*,*) y, "exceeds grid val y"
        endif
